...
httpd_handle_t startWebserver(void)
{
    ...
    config.core_id = 1;
    config.max_open_sockets = 5;    // Do not raise without hardware testing. Tried 8 (with
                                    // CONFIG_LWIP_MAX_SOCKETS=16 for headroom): under concurrent
                                    // load the device wedged - HTTP stopped responding entirely
                                    // while ICMP still worked - where 5 sessions merely truncated
                                    // transfers. More simultaneous sessions cost internal RAM,
                                    // which is the scarce resource here, not sockets.
